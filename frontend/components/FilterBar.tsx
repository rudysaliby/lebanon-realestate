    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 240 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 600, color: t.text }}>
          {valueMin === 0 ? '$0' : fmt(valueMin)} — {valueMax >= SLIDER_MAX ? 'Any' : fmt(valueMax)}
        </span>
      </div>
      <div style={{ position: 'relative', height: 24, display: 'flex', alignItems: 'center' }}>
        ...
        <input
          type="range" min={0} max={SLIDER_MAX} step={STEP}
          value={valueMin} onChange={handleMin}
          style={{
            position: 'absolute', width: '100%', appearance: 'none',
            background: 'transparent', zIndex: valueMin > SLIDER_MAX * 0.9 ? 3 : 2,
            height: 24, cursor: 'pointer', pointerEvents: 'all',
          }}
        />
        {/* Max thumb — higher z-index */}
        <input
          type="range" min={0} max={SLIDER_MAX} step={STEP}
          value={valueMax} onChange={handleMax}
          style={{
            position: 'absolute', width: '100%', appearance: 'none',
            background: 'transparent', zIndex: valueMin > SLIDER_MAX * 0.9 ? 2 : 3,
            height: 24, cursor: 'pointer', pointerEvents: 'all',
          }}
        />
      </div>
      <style>{`
        input[type=range]{cursor:pointer;-webkit-appearance:none}
...
export default function FilterBar({ filters, mode, onChange }: {
  ...
  const regions = [
    ...
    { v: 'South Lebanon', l: 'South' }, { v: 'Bekaa', l: 'Bekaa' },
    { v: 'Nabatieh', l: 'Nabatieh' },
  ]
  ...
  return (
    <div style={{
      ...
      <PriceSlider valueMin={sliderMin} valueMax={sliderMax} onChange={handleSliderChange} t={t} theme={theme} />
